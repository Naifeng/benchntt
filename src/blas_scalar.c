...
__attribute__((always_inline)) void vector_add(uint128_t* c, uint128_t* a, uint128_t* b, uint128_t m){
    __asm volatile("# LLVM-MCA-BEGIN vector add":::"memory");
    for(int i = 0; i < 1024; i++){
        c[i] = addmod128(a[i], b[i], m);
    }
    __asm volatile("# LLVM-MCA-END vector add":::"memory");
}

__attribute__((always_inline)) void vector_sub(uint128_t* c, uint128_t* a, uint128_t* b, uint128_t m){
    __asm volatile("# LLVM-MCA-BEGIN vector sub":::"memory");
    for(int i = 0; i < 1024; i++){
        c[i] = submod128(a[i], b[i], m);
    }
    __asm volatile("# LLVM-MCA-END vector sub":::"memory");
}

__attribute__((always_inline)) void vector_mul(uint128_t* c, uint128_t* a, uint128_t* b, uint128_t m, uint128_t mu){
    __asm volatile("# LLVM-MCA-BEGIN vector mul":::"memory");
    for(int i = 0; i < 1024; i++){
        c[i] = mulmod128(a[i], b[i], m, mu);
    }
    __asm volatile("# LLVM-MCA-END vector mul":::"memory");
}

__attribute__((always_inline)) void axpy(uint128_t* c, uint128_t* a, uint128_t* b, uint128_t x, uint128_t m, uint128_t mu){
    __asm volatile("# LLVM-MCA-BEGIN axpy":::"memory");
    for(int i = 0; i < 1024; i++){
        c[i] = addmod128(a[i], mulmod128(x, b[i], m, mu), m);
    }
...
int main() {
    ...
    uint128_t* twd = (uint128_t*)(malloc(sizeof(uint128_t)*1024));
    uint128_t* x = (uint128_t*)(malloc(sizeof(uint128_t)*1024));
    ...
    uint128_t* vector_add_res = (uint128_t*)(malloc(sizeof(uint128_t)*1024));
    uint128_t* vector_sub_res = (uint128_t*)(malloc(sizeof(uint128_t)*1024));
    uint128_t* vector_mul_res = (uint128_t*)(malloc(sizeof(uint128_t)*1024));
    uint128_t* axpy_res = (uint128_t*)(malloc(sizeof(uint128_t)*1024));
    ...
    for(int i = 0; i < 100; i++){
        ...
        vector_add(vector_add_res, twd, x, m);
    ...
    for(int i = 0; i < 100; i++){
        ...
        vector_sub(vector_sub_res, twd, x, m);
    ...
    for(int i = 0; i < 100; i++){
        ...
        vector_mul(vector_mul_res, twd, x, m, mu);
    ...
    for(int i = 0; i < 100; i++){
        ...
        axpy(axpy_res, twd, x, scalar, m, mu);
    ...
    for(int i = 0; i < 1024; i++){
        if(HI64(vector_add_res[i]) != vvadd_ver[i*2]) {
            printf("add fail: i = %d, add_res = %llu, ver = %llu\n", i*2, HI64(vector_add_res[i]), vvadd_ver[i*2]);
        }
        if(LO64(vector_add_res[i]) != vvadd_ver[i*2+1]) {
            printf("add fail: i = %d, add_res = %llu, ver = %llu\n", i*2+1, LO64(vector_add_res[i]), vvadd_ver[i*2+1]);
        }
        if(HI64(vector_sub_res[i]) != vvsub_ver[i*2]) {
            printf("sub fail: i = %d, sub_res = %llu, ver = %llu\n", i*2, HI64(vector_sub_res[i]), vvsub_ver[i*2]);
        }
        if(LO64(vector_sub_res[i]) != vvsub_ver[i*2+1]) {
            printf("sub fail: i = %d, sub_res = %llu, ver = %llu\n", i*2+1, LO64(vector_sub_res[i]), vvsub_ver[i*2+1]);
        }
        if(HI64(vector_mul_res[i]) != vvmul_ver[i*2]) {
            printf("mul fail: i = %d, mul_res = %llu, ver = %llu\n", i*2, HI64(vector_mul_res[i]), vvmul_ver[i*2]);
        }
        if(LO64(vector_mul_res[i]) != vvmul_ver[i*2+1]) {
            printf("mul fail: i = %d, mul_res = %llu, ver = %llu\n", i*2+1, LO64(vector_mul_res[i]), vvmul_ver[i*2+1]);
        }
        if(HI64(axpy_res[i]) != axpy_ver[i*2]) {
            printf("axpy fail: i = %d, axpy_res = %llu, ver = %llu\n", i*2, HI64(axpy_res[i]), axpy_ver[i*2]);
        }
        if(LO64(axpy_res[i]) != axpy_ver[i*2+1]) {
            printf("axpy fail: i = %d, axpy_res = %llu, ver = %llu\n", i*2+1, LO64(axpy_res[i]), axpy_ver[i*2+1]);
        }
    }
